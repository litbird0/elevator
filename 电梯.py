    ...
    def Cannel(self,aim):      
        if self.list_num[aim]!=-1:       #原来是按下状态
            ...
            if self.direction==1:      
                ...
                for each in self.list_num[aim+1:]:
                    if each>=0:
                        self.top_flag = 0
                if self.top_flag:
                    self.none_flag = 1   #判断已经到达的和要取消的楼层之间有没有楼层之后按下
                    for each in self.list_num[self.now+1:aim]:
                        if each>=0:
                            self.none_flag = 0
                    if self.none_flag:
                        self.list_num[self.now+1] = self.now+1  #比现在到达的高一层的楼层改为按下状态
                ...
                if self.least_flag:
                    ...
                    for each in self.list_num[aim+1:self.now]:
                        if each>=0:
                            self.none_flag = 0
                    if self.none_flag:
                        self.list_num[self.now-1] = self.now-1  #比现在到达的低一层的楼层改为按下状态
 ...
 #计算载重函数
    def Heavy(self,list_add_weight):     #list_add_weight:变化重量的列表
        ...
        #准备超重状态
            ...
            print("忽略电梯外按键")
            self.force_outup = 1     #忽略电梯外后来按下的上行键
            self.force_outdown = 1    #忽略电梯外后来按下的下行键
        else:
            self.force_outup = 0
